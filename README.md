...
- 📂 پشتیبانی از ساختار قدیمی سریال‌ها که فقط لینک پوشه فصل دارند؛ افزونه پوشه فصل را می‌خواند و فایل‌های همان اپیزود را با فرمت‌های مختلف نمایش می‌دهد
...
- 💬 نمایش وضعیت زیرنویس فارسی برای هر لینک/قسمت، مثل `زیرنویس فارسی` یا `بدون زیرنویس فارسی`
...
در بعضی سریال‌ها مثل ساختار قدیمی `Louie`، صفحه اصلی به‌جای لیست مستقیم اپیزودها فقط لینک پوشه هر فصل را دارد؛ مثلاً `دانلود فصل 1` که به مسیر `.../S01/` اشاره می‌کند. در این حالت افزونه پوشه فصل را باز می‌کند، فایل‌هایی مثل `S01E01` را پیدا می‌کند و اگر برای همان اپیزود چند فایل با کیفیت/فرمت متفاوت وجود داشته باشد، همه آن‌ها را به‌صورت جداگانه در Stremio نمایش می‌دهد.
...
```json
{
  "name": "WEB-DL 4K 2160p 10bit HDR • دوبله • زیرنویس فارسی",
  "title": "S1E2 - WEB-DL 4K 2160p 10bit HDR • encoder: PSA • زیرنویس فارسی",
  "url": "https://example.com/video/file.mkv"
}
```

اگر در متن آیتم دانلود عبارتی مثل `کیفیت : WEB-DL 4K 2160p 10bit HDR` وجود داشته باشد، پیشوند `کیفیت :` حذف می‌شود و فقط مقدار کیفیت به Stremio ارسال می‌شود. فیلد `name` شامل فرمت/کیفیت، وضعیت دوبله و وضعیت زیرنویس فارسی است؛ اما اطلاعات اضافی مثل `encoder` یا عبارت‌های نامرتبط مثل `میانگین امتیاز` وارد آن نمی‌شود.

اگر `انکودر : PSA` یا `Encoder: PSA` پیدا شود، به شکل `encoder: PSA` فقط در `title` استریم اضافه می‌شود. اگر متن‌هایی مثل `زیرنویس فارسی`، `با زیرنویس`، `بدون زیرنویس` یا `زیرنویس فارسی : ندارد` پیدا شود، وضعیت زیرنویس همان لینک/قسمت هم در `name` و هم در `title` نمایش داده می‌شود. برای فیلم‌ها، `title` معمولاً کیفیت، انکودر و وضعیت زیرنویس است؛ برای سریال‌ها، شماره فصل/قسمت هم در `title` قرار می‌گیرد.
