...
### خطای `EADDRINUSE` یا اشغال بودن پورت

این خطا یعنی پورت تنظیم‌شده، معمولاً `8000`، قبلاً توسط یک برنامه دیگر استفاده شده است. یا آن برنامه را متوقف کنید، یا افزونه را با پورت دیگری اجرا کنید:

```bash
PORT=8001 npm start
```

اگر از فایل `.env` استفاده می‌کنید، مقدار `PORT` را تغییر دهید:
